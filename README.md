...
Static-only hosting can serve the manual editor but cannot securely provide AI generation. Production deployments need a Node-compatible process or an equivalent secure server-function adaptation for the same-origin API route.
...
- The in-memory AI limiter does not coordinate quota across multiple processes or serverless instances.
